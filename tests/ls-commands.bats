...
@test "ls-commands: list commands" {
    ...
    assert_equal "  ls-cache        List locally cached verions (no need to download & build)"   "${lines[5]}" 
    assert_equal "  ls-commands     List available commands"                                     "${lines[6]}" 
    assert_equal "  ls-envs         List environments"                                           "${lines[7]}" 
